...
if "__main__" == __name__:
    ...
    # 加载tokenizer
    tokenizer = AutoTokenizer.from_pretrained("D:\\model\\chatglm3-6b-32k", trust_remote_code=True)
    # 将数据集变化为token形式
    tokenized_ds = ds.map(process_func, remove_columns=ds.column_names)

    # 创建模型
    model = AutoModelForCausalLM.from_pretrained("D:\\model\\chatglm3-6b-32k",torch_dtype=torch.half, trust_remote_code=True, low_cpu_mem_usage=True)
