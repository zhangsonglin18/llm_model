...
if "__main__" == __name__:
    ...
    # 加载 tokenizer
    tokenizer = AutoTokenizer.from_pretrained('D:\\model\\Yi-6B-Chat', use_fast=False, trust_remote_code=True)
    # # 将数据集变化为token形式
    tokenized_id = ds.map(process_func, remove_columns=ds.column_names)
    
    # 创建模型并以半精度形式加载，如果你的显卡比较新的话，可以用torch.bfolat形式加载
    model = AutoModelForCausalLM.from_pretrained('D:\\model\\Yi-6B-Chat', trust_remote_code=True, torch_dtype=torch.half, device_map="auto")
